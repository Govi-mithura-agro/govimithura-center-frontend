...
const ResizeMapScreen = () => {
  ...
  useEffect(() => {
    axios.get(`/api/auth/mapTemplate/getOneTemplate/${templateId}`)
      .then((response) => {
  ...
  const saveMapPoints = async () => {
    try {
      ...
      if (!appArray || appArray.length === 0 || !appArray[0].path) {
        console.error("appArray is not defined or is empty");
      ...
      if (response.status === 200) {
        ...
        console.error("Failed to update location. Status code:", response.status);
        message.error("Failed to update location. Please try again.");
      }
    } catch (error) {
