...
function FarmersList() {
  ...
  const saveEditManager = async () => {
    ...
    const managerData = {
      managerId: tableModelContent.managerId, // Ensure you send the correct ID field
      address: editAddress,
      dob: editDob,
      name: editFullName,
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/managers/editManager", // Adjust endpoint as needed
        managerData
      );
      message.success("Manager edited successfully");
      setTableModelOpen(false);
      fetchManagersList(); // Fetch updated manager list
    } catch (error) {
  ...
  const saveManager = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/managers/addManager",
        managerData
      );
      ...
      message.error(error.response?.data?.message || "An error occurred while adding the manager");
    }
  };
  ...
  useEffect(() => {
    let tempList = employeeList; // Assuming employeeList is your list of farmers

    if (searchKey && searchKey !== "") {
      tempList = tempList.filter(
        (item) =>
          item.fullname.toLowerCase().includes(searchKey) ||
          item.idnumber.toLowerCase().includes(searchKey) ||
