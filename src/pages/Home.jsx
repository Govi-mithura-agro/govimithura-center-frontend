  ...
  const handleLabelClick = useCallback(
    async (mapId) => {
      if (mapId === selectedMapId) {
        ...
        try {
          const response = await axios.get(`http://localhost:5000/api/mapTemplate/${mapId}`);
          // Check if response data is correct and contains templateName
          if (response.data && response.data.templateName) {
            setSelectedMapDetails(response.data);
          } else {
            console.error("Invalid map details received:", response.data);
            setSelectedMapDetails(null);
          }
        } catch (error) {
